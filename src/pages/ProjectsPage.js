  ...
  return (
    <MainLayout>
      {!!projectList && projectList.length > 0 && (
        <>
          <div className={"projectListingTitle"}>PROJECTS - {projectList[0].conceptCategory}</div>
          <div className={"projectListingContainer"}>
            {projectList.map((project, index) => {
              return (
                  <div className={"projectName"} key={`projectTitle_${index}`}>
                    <Link to={`/project/${project.id}`} style={{ textDecoration: "none" }} >
                    {project.projectTitle} - <span className={'authors'}>{project.principleName}{project.otherNames && project.otherNames !== "" ? `, ${project.otherNames}`: "" }</span>
                    </Link>
                  </div>
              );
            })}
